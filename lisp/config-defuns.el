...
If a buffer is not file and not dired, copy value of `default-directory' (which is
usually the “current” dir when that buffer was created)
...
(defun my/goto-baseclass ()
  ...
  (let ((baseclass-regex (cond ((eq major-mode 'python-mode) "\\bclass \\w+?(\\(?1:\\w+\\))")
                               ((eq major-mode 'java-mode) "\\bclass [a-zA-Z0-9<>]+?[ \n\t\r]+extends \\(?1:\\w+\\)")
